function [table_with_new_proportions] = match_proportions_and_return_updated_table_4(human_data_table,array_of_propotions_to_match_to)
...
number_of_rows_to_get = round(.5 * height(human_data_table));
for i=1:length(unique_clusters)
    curr_clust = unique_clusters(i);
    table_with_only_curr_clust = human_data_table(human_data_table.cluster_number == curr_clust,:);
    sample = datasample(table_with_only_curr_clust,round(array_of_propotions_to_match_to(i)*number_of_rows_to_get),1);
