function [] = create_heat_maps_for_mean_three_by_one(data_table,dir_to_save_figs_to,do_the_simplifcation_or_dont)
    ...
    function [] = create_subplot(do_the_simplifcation_or_dont,simple_array_cell,unique_experiments,dimensions_array,which_dimension,k)
        ...
        if do_the_simplifcation_or_dont
            title([strcat("Change in Mean Cluster ",string(k)," Across All Experiments For ", dimensions_array(which_dimension)),"by 2 sample T test", "0: Not Significant"])
        else
            title([strcat("Sig. From Comparing Mean for Cluster ",string(k)," Across All Experiments For", dimensions_array(which_dimension)),"Significance determined by 2 sample T test"])
        end
        xlabel("experiment")
        ylabel("experiment")
        set(gcf,'renderer','Painters');
    end
    function [] = create_the_plot(map_of_signifance,unique_clusters,unique_experiments,which_dimension,do_the_simplifcation_or_dont,dir_to_save_figs_to)
        simple_array_cell_1 = turn_map_into_simple_arrays(map_of_signifance,unique_experiments,unique_clusters,1);
        simple_array_cell_2 = turn_map_into_simple_arrays(map_of_signifance,unique_experiments,unique_clusters,2);
        simple_array_cell_3 = turn_map_into_simple_arrays(map_of_signifance,unique_experiments,unique_clusters,3);
        dimensions_array = ["log(abs(max))","log(abs(shift))","log(abs(slope))"];
        for k=1:length(unique_clusters)
            figure('units','normalized','outerposition',[0 0 1 1])
            subplot(1,3,1);
            create_subplot(do_the_simplifcation_or_dont,simple_array_cell_1,unique_experiments,dimensions_array,1,k);
            subplot(1,3,2);
            create_subplot(do_the_simplifcation_or_dont,simple_array_cell_2,unique_experiments,dimensions_array,2,k);
            subplot(1,3,3);
            create_subplot(do_the_simplifcation_or_dont,simple_array_cell_3,unique_experiments,dimensions_array,3,k);
            sgtitle(strcat("Cluster",string(k)));
            saveas(gcf,strcat(dir_to_save_figs_to,"\Cluster ",string(k),".fig"),"fig")
...
create_the_plot(map_of_signifance,unique_clusters,unique_experiments,1,do_the_simplifcation_or_dont,dir_to_save_figs_to)
