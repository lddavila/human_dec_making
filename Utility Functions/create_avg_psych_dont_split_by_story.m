function [all_ys]  = create_avg_psych_dont_split_by_story(sesh_data, psych_to_cluster, type, same_scale, save_to)
...
figure;
psych_to_cluster = renamevars(psych_to_cluster,["experiment"],["story_type"]);
...
y_min = 100;
y_max = 0;
all_ys = cell2table(cell(0,5),'VariableNames',["y1","y2","y3","y4","cluster_number"]);
for i = 1 : num_clusters
    ...
    merge = outerjoin(sesh_info,sesh_data,'Keys',{'cost','story_num','subjectidnumber','story_type'});
    ...
    nah = sprintf("Cluster %d, y1:%0.3f,y2:%0.3f,y3:%0.3f,y4:%0.3f",cluster,y(1),y(2),y(3),y(4));
    all_ys = [all_ys;table(y(1),y(2),y(3),y(4),i,'VariableNames',["y1","y2","y3","y4","cluster_number"])];
    ...
    labels_of_data = {};
    for p=1:length(x)
        labels_of_data{end+1} = char(strcat(string(round(x(p),4)),",",string(round(y(p),4))));
    end
    title(["cluster " + string(i) + " "+type_of_fit, "R SQ" + gof_to_be_returned.rsquare])
    text(x,y,labels_of_data);
