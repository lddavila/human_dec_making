function [] = filter_based_on_individual_meta_data(human_data_table,all_data,all_data_order,colors,version_name,threshold,what_percentage_must_be_significant,surrounding_directory,filter_to_use)
...
for the_threshold=0:10:100
    for preference_threshold=0:10:100
        ...
        dir_to_save_figs_to = strcat("3d Cluster Plots ",filter_to_use ," split at ",string(the_threshold)," Story Pref Min ", string(preference_threshold), "chi squared ",string(round(p,3)));
        if found_significant_changes
            human_lower_bound_stats_map = get_human_data_table_stats(table_of_lower_thresholds);
            create3d_cluster_plot_for_human_hunger(table_of_lower_thresholds,colors,"Human","Lower Bound",dir_to_save_figs_to,human_lower_bound_stats_map,version_name,strcat(filter_to_use," <",string(the_threshold)," Story Prefs >=", string(preference_threshold)),unique(human_data_table.cluster_number),p);
            human_upper_bound_stats_map = get_human_data_table_stats(table_of_upper_thresholds);
            create3d_cluster_plot_for_human_hunger(table_of_upper_thresholds,colors,"Human","Upper Bound",dir_to_save_figs_to,human_upper_bound_stats_map,version_name,strcat(filter_to_use," >=",string(the_threshold), " Story Prefs >=", string(preference_threshold)),unique(human_data_table.cluster_number),p);
