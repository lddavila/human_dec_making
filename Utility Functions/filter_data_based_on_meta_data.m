function [] = filter_data_based_on_meta_data(human_data_table,all_data,all_data_order,colors,version_name,threshold,what_percentage_must_be_significant,surrounding_directory,filters_to_use,story_prefs_min)
%filters to use: string array, tells the function what meta data you want to filter by, should be the same as the columns in the table_with_hunger variable
    %current acceptable filters: experiment, average_hunger, tiredness, sex, age, pain
%story_pref_min: int, tells the function what the min story_pref should be used, filtering out anyting lower than this threshold
...
%step 1: check if there are any filters in your list
%step 2: if there ARE filters in your list start doing a bunch of permutations 
%       (increments of 10 if it's numerical, by unique string if it's any other data type)


    function [] = recursively_filter_data(filters_to_use,data_table,the_filter_to_be_used,filters_that_have_been_applied)
        if ~isempty(filters_to_use)
            current_filter = filters_to_use(1);
            if class(data_table.(current_filter)) == "string"
                unique_permutations = unique(data_table.(current_filter));
            else
                unique_permutations = 0:10:100;
            end
            for v=unique_permutations
...
% for hunger_threshold=10:10:100
%     for tiredness_threshold=60:10:100
%         for pain_threshold=10:10:100
