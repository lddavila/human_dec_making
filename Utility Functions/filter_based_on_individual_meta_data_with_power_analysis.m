function [] = filter_based_on_individual_meta_data_with_power_analysis(human_data_table,all_data,all_data_order,colors,version_name,threshold,what_percentage_must_be_significant,surrounding_directory,filter_to_use)
    ...
    function [] = create_heat_map_of_significant_changes(heat_map_matrix_mean,heat_map_matrix_var,y_labels,hunger_threshold,p,number_of_dp_in_lower_threshold,number_of_dp_in_upper_threshold,threshold,dir_to_save_figs_to,preference_threshold,filter_to_use)
        % colormap(flip(sky));
        ...
        title(["Significant Changes In Mean Determined By ttest2", ...
            strcat("Split At ",string(filter_to_use),":", string(hunger_threshold)," Story Pref: ",string(preference_threshold)), ...
        ...
        title(["Significant Changes In Variance Determined By vartest2", ...
            strcat("Split At ",string(filter_to_use),":", string(hunger_threshold)), ...
            strcat("Significant Change In Density Determined by Chi Squared:",string(p)), ...
            strcat("# of Data Points In lower threshold:",string(number_of_dp_in_lower_threshold)," upper threshold:",string(number_of_dp_in_upper_threshold)), ...
...
for the_gender =1: length(unique_genders)
    current_gender = unique_genders(the_gender);
    for the_threshold=0:10:100
        for preference_threshold=0:10:100
            table_of_upper_thresholds = table_with_hunger(table_with_hunger.(filter_to_use) >= the_threshold & table_with_hunger.story_prefs >= preference_threshold & strcmpi(string(table_with_hunger.sex),current_gender),:);
            table_of_lower_thresholds = table_with_hunger(table_with_hunger.(filter_to_use) < the_threshold & table_with_hunger.story_prefs >= preference_threshold & strcmpi(string(table_with_hunger.sex),current_gender),:);
