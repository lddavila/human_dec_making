function [] = create_3d_bar_plots_for_variance(data_table,dir_to_save_figs_to,do_the_simplifcation_or_dont)
    ...
    function [] = create_the_plot(simple_array_cell,cell_array_of_variances,unique_clusters,unique_experiments,which_dimension,do_the_simplifcation_or_dont,dir_to_save_figs_to)
        for k=1:length(unique_clusters)
            ...
            if do_the_simplifcation_or_dont
                ...
                b= bar3(make_diagonal_nan(simple_array_cell{k}));
            end
            colorbar;
            for p=1:length(b)
                zdata = b(p).ZData;
            ...
            title(strcat("Sig. From Comparing Variance for Cluster ",string(k)," Across All Experiments For", dimensions_array(which_dimension)))
            subtitle("Significance determined by 2 sample F test")
            xlabel("experiment")
            ylabel("experiment")
