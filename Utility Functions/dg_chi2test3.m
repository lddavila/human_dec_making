function [significance, colsNotMrgd, finalCols, colsMrgdA, colsMrgdB] = ...
    dg_chi2test3(NothingMatrix, userules)
...
colsMrgdA = [];
colsMrgdB = [];
colsNotMrgd = 1:size(NothingMatrix,2);  % These get deleted in parallel with cols of N
if any(any(isnan(NothingMatrix)))
    significance = NaN;
    return
end
if nargin < 2
...
Ntot = sum(sum(NothingMatrix));
columnP = sum(NothingMatrix,1)/Ntot;
rowP = sum(NothingMatrix,2)/Ntot;
P = repmat(columnP, size(NothingMatrix,1), 1) .* repmat(rowP, 1, size(NothingMatrix,2));
expected = P * Ntot;
...
if userules == 1
    ...
    if any(any(expected < 1))
        ...
        significance = NaN;
        return
    elseif sum(sum(expected < 5)) > numel(NothingMatrix)/5
...
elseif userules == 2
    ...
    if any(any(toosmall))
        ...
        mergedCol = sum(NothingMatrix(:,toosmallcols),2);   % column vector
        if any((sum(mergedCol,1) * rowP) < 1)
            ...
            mergedCol = sum(NothingMatrix(:,colsMrgdA),2);
            NothingMatrix(:,colsMrgdA) = [];
            NothingMatrix(:,end+1) = mergedCol;
            colsNotMrgd(colsMrgdA(colsMrgdA<=length(colsNotMrgd))) = [];
        else
            % simply merge the "toosmall" columns:
            NothingMatrix(:,toosmallcols) = [];
            NothingMatrix(:,end+1) = mergedCol;
    ...
    while sum(sum(toosmall)) > numel(NothingMatrix)/5
        if size(NothingMatrix,2) < 3
        ...
        ix = IX(1:2);
        colsMrgdB(end+1,:) = ix;
        mergedCol = sum(NothingMatrix(:,ix), 2);
        NothingMatrix(:,ix) = [];
        NothingMatrix(:,end+1) = mergedCol;
...
df = numel(NothingMatrix) - (size(NothingMatrix,1) + size(NothingMatrix,2) - 2) - 1;

% Calculate the chi^2 estimate:
if df == 1
    % Use Yates correction
    diffs = abs(NothingMatrix - expected) - 0.5;
else
    % No Yates correction
    diffs = NothingMatrix - expected;
end
