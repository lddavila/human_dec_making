function call_spectral_clustering_combine_all_human_data_2(table_of_dir,directory_where_cluster_table_should_be_saved,epsilon,given_number_of_clusters,colors,method)
table_of_data = cell2table(cell(0,6),"VariableNames",["A","B","C","D","E","F"]);
...
for i=1:height(table_of_dir)
    current_table = stoppingPointsSigmoidClustering(1,table_of_dir{i,2});
    F = repelem(table_of_dir{i,1},height(current_table),1);
    F = table(F);
    current_table = [current_table,F];
...
for j=1:length(unique_indexes)
    ...
    three_d_cluster_table = getClusterTable3dWithExperiment(xVsYVsZ,labels,index,unique_indexes(j),table_of_data.F);
