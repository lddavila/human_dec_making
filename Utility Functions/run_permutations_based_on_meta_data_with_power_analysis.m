function [] = run_permutations_based_on_meta_data_with_power_analysis(human_data_table,all_data,all_data_order,colors,version_name,threshold,what_percentage_must_be_significant,surrounding_directory)
    ...
    function [significant_changes,var_significance_matrix,mean_significance_matrix,y_labels,p,pwrout_for_x,pwrout_for_y,pwrout_for_z,resized_lower_bound_table,resized_upper_bound_table] = check_for_significant_differences(lower_bound_table,upper_bound_table,unique_clusters,threshold,what_percentage_must_be_significant,hunger_threshold,tiredness_threshold,pain_threshold,preference_threshold)
        ...
        var_of_entire_lower_threshold_data_set = var([resized_lower_bound_table.clusterX,resized_lower_bound_table.clusterY,resized_lower_bound_table.clusterZ]);
        var_of_entire_upper_threshold_data_set = var([resized_upper_bound_table.clusterX,resized_upper_bound_table.clusterY,resized_upper_bound_table.clusterZ]);
    ...
    function [] = create_heat_map_of_significant_changes(heat_map_matrix_mean,heat_map_matrix_var,y_labels,hunger_threshold,tiredness_threshold,pain_threshold,p,number_of_dp_in_lower_threshold,number_of_dp_in_upper_threshold,threshold,dir_to_save_figs_to,preference_threshold)
        ...
        title(["Significant Changes In Mean Determined By ttest2", ...
            strcat("Split At Hunger:", string(hunger_threshold)," Tiredness: ",string(tiredness_threshold)," Pain: ",string(pain_threshold)," Story Pref: ",string(preference_threshold)), ...
    ...
    for tiredness_threshold=0:10:100
        for pain_threshold=0:10:100
            for preference_threshold=0:10:100
                table_of_upper_thresholds = table_with_hunger(table_with_hunger.average_hunger >= hunger_threshold & table_with_hunger.tiredness >= tiredness_threshold & table_with_hunger.pain >= pain_threshold & table_with_hunger.story_prefs >= preference_threshold,:);
                table_of_lower_thresholds = table_with_hunger(table_with_hunger.average_hunger < hunger_threshold & table_with_hunger.tiredness < tiredness_threshold & table_with_hunger.pain < pain_threshold& table_with_hunger.story_prefs >= preference_threshold,:);
                ...
                [found_significant_changes,var_significance_matrix,mean_significance_matrix,y_labels,p,pwrout_for_x,pwrout_for_y,pwrout_for_z,resized_lower_bound_table,resized_upper_bound_table] = check_for_significant_differences(table_of_lower_thresholds,table_of_upper_thresholds,unique(human_data_table.cluster_number),threshold,what_percentage_must_be_significant,hunger_threshold,tiredness_threshold,pain_threshold,preference_threshold);
                ...
                if found_significant_changes
                    ...
                    create3d_cluster_plot_for_human_data_with_power(table_of_lower_thresholds,colors,"Human","Lower Bound",dir_to_save_figs_to,human_lower_bound_stats_map,version_name,strcat("Hunger <",string(hunger_threshold)," Tiredness <",string(tiredness_threshold)," Pain <",string(pain_threshold), " Story Prefs >", string(preference_threshold)),unique(human_data_table.cluster_number),p,power_string);
                    human_upper_bound_stats_map = get_human_data_table_stats(table_of_upper_thresholds);
                    create3d_cluster_plot_for_human_data_with_power(table_of_upper_thresholds,colors,"Human","Upper Bound",dir_to_save_figs_to,human_upper_bound_stats_map,version_name,strcat("Hunger >",string(hunger_threshold)," Tiredness >",string(tiredness_threshold)," Pain >",string(pain_threshold), " Story Prefs >", string(preference_threshold)),unique(human_data_table.cluster_number),p,power_string);
