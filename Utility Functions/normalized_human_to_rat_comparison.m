function normalized_human_to_rat_comparison(name_of_rat_file,human_cluster_table,directory_of_rat_file,b_dist_plots_dir,colors,human_stats_map,rat_stats_map,date_created,version_name)
...
for i=1:length(rat_clusters)
    ...
    %     rat_stats_map('Number Of Unique Subjects'), rat_stats_map('Number of Data Points')), ...
    %     strcat("Date Created:",string(datetime("today",'Format','MM-d-yyyy'))), ...
    %     "Created by normalized\_human\_to\_rat\_comparison.m", ...
    %     version_name])
    % set(gcf,'renderer','Painters');
