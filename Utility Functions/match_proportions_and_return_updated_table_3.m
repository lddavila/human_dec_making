...
number_of_rows_to_take = round(height(only_experiment_to_keep)*.5);
for i=1:length(unique_clusters)
    ...
    if height(table_with_only_curr_clust)==0
        sample = datasample(table_with_only_curr_clust,0);
    else
    sample = datasample(table_with_only_curr_clust,round(array_of_propotions_to_match_to(i)*number_of_rows_to_take),1);
    end
    clc;
    disp(sample);
