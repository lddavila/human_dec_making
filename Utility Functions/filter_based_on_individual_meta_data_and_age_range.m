function [] = filter_based_on_individual_meta_data_and_age_range(human_data_table,all_data,all_data_order,colors,version_name,threshold,what_percentage_must_be_significant,surrounding_directory,filter_to_use)
...
unique_age_ranges = unique(table_with_hunger.age);
for the_age_range=1:length(unique_age_ranges)
    age_range_to_use = unique_age_ranges(the_age_range);
    for the_threshold=0:10:100
        for preference_threshold=0:10:100
            table_of_upper_thresholds = table_with_hunger(table_with_hunger.(filter_to_use) >= the_threshold & table_with_hunger.story_prefs >= preference_threshold & strcmpi(table_with_hunger.age,age_range_to_use),:);
            table_of_lower_thresholds = table_with_hunger(table_with_hunger.(filter_to_use) < the_threshold & table_with_hunger.story_prefs >= preference_threshold & strcmpi(table_with_hunger.age,age_range_to_use),:);
            ...
            dir_to_save_figs_to = strcat(age_range_to_use," 3d Cluster Plots ",filter_to_use ," split at ",string(the_threshold)," Story Pref Min ", string(preference_threshold), "chi squared ",string(round(p,3)));
            if found_significant_changes
                human_lower_bound_stats_map = get_human_data_table_stats(table_of_lower_thresholds);
                create3d_cluster_plot_for_human_hunger(table_of_lower_thresholds,colors,"Human",strcat(age_range_to_use," Lower Bound"),dir_to_save_figs_to,human_lower_bound_stats_map,version_name,strcat(filter_to_use," <",string(the_threshold)," Story Prefs >=", string(preference_threshold)),unique(human_data_table.cluster_number),p);
                human_upper_bound_stats_map = get_human_data_table_stats(table_of_upper_thresholds);
                create3d_cluster_plot_for_human_hunger(table_of_upper_thresholds,colors,"Human",strcat(age_range_to_use," Upper Bound"),dir_to_save_figs_to,human_upper_bound_stats_map,version_name,strcat(filter_to_use," >=",string(the_threshold), " Story Prefs >=", string(preference_threshold)),unique(human_data_table.cluster_number),p);
