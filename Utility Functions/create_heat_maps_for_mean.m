function [] = create_heat_maps_for_mean(data_table,dir_to_save_figs_to,do_the_simplifcation_or_dont)
    ...
    function [] = create_the_plot(simple_array_cell,cell_array_of_variances,unique_clusters,unique_experiments,which_dimension,do_the_simplifcation_or_dont,dir_to_save_figs_to)
        for k=1:length(unique_clusters)
            ...
            if do_the_simplifcation_or_dont
                title([strcat("Significant change in Mean or not Cluster ",string(k)," Across All Experiments For ", dimensions_array(which_dimension)),"Significance determined by 2 sample T test", "0: Not Significant"])
            else
