function [] = create_mean_spider_plots_same_cluster_different_story(data_table,dir_to_save_figs_to)
...
for i=1:length(unique_clusters)
    ...
    for j=1:length(unique_experiments)
        ...
        array_of_means = [array_of_means;mean(only_current_xyz)];
    end
    plot_spider_plots_ontop_of_eachother_given_axes_labels_for_mean(array_of_means, ...
        ["mean(log(abs(max)))","mean(log(abs(shift)))","mean(log(abs(slope)))"], ...
        strcat("Mean Of Cluster ",string(curr_cluster), " Across All Experiments" ), ...
        legend_strings, ...
        dir_to_save_figs_to);
end
