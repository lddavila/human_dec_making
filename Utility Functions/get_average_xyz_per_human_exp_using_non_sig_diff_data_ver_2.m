function [] =get_average_xyz_per_human_exp_using_non_sig_diff_data_ver_2(human_data_table, C,dir_to_save_figs_to,human_stats_map,weights,map_of_mean_significance,version_name)
...
for s=1:length(story_types)
    ...
    array_of_std_errors = [];
    for p=1:1000
        non_sig_different_subset = match_proportions_and_return_updated_table_5(randomized_data,get_cluster_counts(human_data_table(strcmpi(string(human_data_table.experiment),string(story_type)),:),unique_clusters));
        data_to_examine = [non_sig_different_subset.clusterX,non_sig_different_subset.clusterY,non_sig_different_subset.clusterZ];
        array_of_means = [array_of_means;mean(data_to_examine)];
        array_of_std_errors = [array_of_std_errors;((std(data_to_examine))/sqrt(size(data_to_examine,1)))];
    end


    
    disp("Mean")
    xyz_mean = mean(array_of_means);
    disp(xyz_mean);
    xyz_std_error = mean(array_of_std_errors);
    disp("std error")
    disp(xyz_std_error)
...
title([title_strings,"Created Using Non Significantly Differnt Data",...
    strcat("Date Created:",string(datetime("today",'Format','MM-d-yyyy'))), ...
    "Created By get\_average\_xyz\_per\_human\_exp\_using\_non\_sig\_diff\_data\_ver\_2.m", ...
    version_name]);
