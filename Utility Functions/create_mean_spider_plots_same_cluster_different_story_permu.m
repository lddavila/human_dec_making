function [] = create_mean_spider_plots_same_cluster_different_story_permu(data_table,dir_to_save_figs_to)
...
for i=1:length(unique_clusters)
    ...
    for j=1:length(unique_experiments)
        ...
        curr_exp_mean = mean(only_current_xyz);
        for k=j+1:length(unique_experiments)
            ...
            curr_exp_mean_2 = var(only_curr_xyz_2);
           
            [h,p] = ttest2(only_current_xyz,only_curr_xyz_2);
            plot_spider_plots_ontop_of_eachother_include_sig_for_mean([curr_exp_mean;curr_exp_mean_2], ...
                ["mean(log(abs(max)))","mean(log(abs(shift)))","mean(log(abs(slope)))"], ...
                strcat("Mean Of Cluster ",string(curr_cluster), " Between ",curr_exp," and ",curr_exp_2 ), ...
                [curr_exp,curr_exp_2], ...
                dir_to_save_figs_to,sprintf("max->p:%.3f h:%d shift->p:%.3f h:%d slope->p:%.3f h:%d \n From 2 Sample T Test",p(1),h(1),p(2),h(2),p(3),h(3)));
        end
    end
