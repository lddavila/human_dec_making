function [] = create_a_bhaat_dist_tree_comparing_rat_to_human(human_data_table,human_stats_map,rat_data_table,rat_stats_map,version_name,dir_to_save_figs_to,the_title)
...
array_of_bhaat_distance = zeros(length(rat_clusters)+length(human_clusters),length(human_clusters)+length(rat_clusters));
for i=1:length(human_clusters)
    ...
    for j=1:length(rat_clusters)
        ...
        array_of_bhaat_distance(i,j+length(human_clusters)) = mean(bhattacharyyaDistance([curr_hum_xyz;curr_rat_xyz],[hum_labels;rat_labels]));
        array_of_bhaat_distance(j+length(human_clusters),i) = mean(bhattacharyyaDistance([curr_hum_xyz;curr_rat_xyz],[hum_labels;rat_labels]));
    end
end

cluster_labels = [];

for i=1:(length(rat_clusters) + length(human_clusters))
    if i<=length(human_clusters)
        cluster_labels = [cluster_labels;strcat("Human Cluster ",string(i))];
    else
        cluster_labels = [cluster_labels;strcat("Rat Cluster", string(i-length(human_clusters)))];
    end
end
