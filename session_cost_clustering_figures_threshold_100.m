...
%% SET UP PART 2: (DOESNT NEED TO BE RUN AGAIN)copy only the data which meets the threshold
...
%% SET UP PART 3: (DOESNT NEED TO BE RUN AGAIN) run fcm for human data (session cost clustering) 
...
%% SET UP PART 4: put rat and human data into a table to be used
...
%% SET UP PART 5: get human data information (ie. # of subjects, # of data points )
human_stats_map = get_human_data_table_stats(human_data_table);
rat_stats_map = get_rat_data_table_stats(rat_data_table);
%% FIG 1L LEFT PANEL: create the average xyz plot
C = ['r','g','b','c'];
...
%% FIG 1L RIGHT PANEL: try to recreate xyz using data which is not significantly different data from each task (SF3Q) version 2
clc;
C = ['r','g','b','c'];
...
%% Supplemental Figure 4M: recreate average xyz plots using explicitly wrong proportions
% clc;
C = ['r','g','b','c'];
proportions_to_match = get_cluster_counts_2(human_data_table,unique(human_data_table.experiment),unique(human_data_table.cluster_number),1);
% disp([keys(proportions_to_match).',values(proportions_to_match).'])
