...
addpath(pwd)
cd("..")
version_name = " for session_cost_clustering";
%% run fcm for human data (session cost clustering) 
table_of_human_dir = get_dirs_with_data("session_cost_clustering");
...
% call_FCM_for_all_human_data(table_of_human_dir,centers, strcat("Cluster Table For session_cost_clustering created on ",string(datetime("today",'Format','MM-d-yyyy'))))
...
human_data_table = return_given_cluster_table("all human data.xlsx","Cluster Table For session_cost_clustering created on 05-6-2024");
...
draw_a_shape_around_each_cluster(human_data_table,  human_stats_map,"human","All","cluster shape plots",0,colors);
...
%% drop shape around each cluster for rat and human
colors = distinguishable_colors(30);
...
%% drop shape around each cluster for rat and human
colors = distinguishable_colors(30);
draw_a_shape_around_cluster_rat_and_human(only_aa_data,rat_data_table,"human and rat",colors)
